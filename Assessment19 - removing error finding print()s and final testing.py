...
class Main_window_and_program:
     ...
     def right(self):
          ...
          self.question_marks.append(self.edit_line+'='+self.answer+' : CORRECT \n')
     ...
     def wrong(self): 
          ...
          self.question_marks.append(self.edit_line+'='+self.answer+' : WRONG \n')
     ...
     def answers(self):
          ...
          #different levels need different equal checks and this accounts for every level below 4. The levels below four utilise python's basic calculation in order to compare to user answers whereas the other levels use a third party
          #library in order to differentiate the question or use a list of algebra answers that is generated. 
          try: 
               if self.level < 4:
                    #checks if answer = python answer to two decimal places (with the %.2f) without that differences between python calculation and individual calculation may be different and would decrease user functionality having to
                    #enter in so many decimal places
          ...
          #only applies to level 4, which is algebra. This is because algebra questions require that their answers are appended, Python cannot calculate the algebra in order to compare to a user's answer without use of a third party
          #library. 
          if self.level == 4:
               #checks if the according answer (identified through question_counter) that are appended as they are randomly generated is the same as the user's answer. Has to use -2 because the question counter intially starts with an
               #index of two, therefore would normally give the answer as two ahead of the question
               try: 
          ...
          while True:
               try: 
                    ...
                    #right answer counter for results reset
